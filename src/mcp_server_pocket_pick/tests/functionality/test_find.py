...
def test_fts_query(populated_db):
    # Test with FTS mode
    command = FindCommand(
        text="programming",
        mode="fts",
    ...
    # This should use the FTS query path if available
    results = find(command)
    
    # Should find results with "programming"
    assert len(results) >= 1
    assert any("programming" in item.text.lower() for item in results)

def test_find_invalid_db(temp_db_path):
    ...
    # Should raise an exception with corrupted DB
    with pytest.raises(Exception):
        find(command)

def test_find_empty_db(temp_db_path):
    # Initialize empty DB
